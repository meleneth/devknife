...
## Artifact Families (Current And Planned)

- project definition (planned)
- workflow definitions (current)
- reusable operation definitions (planned; operations are currently inline effects)
- environment definitions (current)
- event schema references (planned)
- run traces (current generated artifacts)
...
YAML is the initial human-authored workflow format. The format is intentionally small and versioned as `devknife.workflow/v1alpha1`. The pipeline is YAML -> parsed config structs -> validation -> `Workflow` -> planning or engine execution.
