...
Implementation note: an initial desktop shell now exists in `apps/desktop`. It lists example workflows, requests plans from `devknife-core`, calls the engine through Tauri commands, renders the returned trace/report in Vue, and provides a repository-confined read-only YAML source inspector.
